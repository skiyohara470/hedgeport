...
- 別ウィンドウのプレビュー画面は実ファイルを表示する（local/remote、読み取り専用）。既定 Open または Open… > Preview が `openPreview(request)` を呼んで独立ウィンドウを開き、main は session（送信元ウィンドウ束縛）越しに `readTextFile`/`readLocalText` を再利用して読む。認証情報・ローカル絶対パスは URL/hash/query に載せない。ウィンドウ内検索は Ctrl/Cmd+F。編集は Open… > Built-in Editor でファイラー内の編集モーダルを開き、保存後に一覧を再ロードする（preview は読み取り専用なので保存はしない）。プレビューウィンドウの diff は未実装。
