...
- `windowChrome.ts`
  ウィンドウのタイトルバー外観をプラットフォーム別に決める純関数 `windowChromeOptions(process.platform)`（electron 非依存・単体テスト可）。macOS は `titleBarStyle: 'hiddenInset'` + `trafficLightPosition`（native traffic lights 維持）、Windows / Linux は native frame 維持のため空を返す。`index.ts` が main / preview 双方の `BrowserWindow` へ spread し、各ウィンドウの `enter/leave-full-screen` を `window:fullscreen` で renderer へ通知する（全画面時の左余白畳み込み用）。
- `settingsStore.ts`
...
- `features/platform/platform.ts`
  preload 公開の `platform`（UA 非依存の読み取り専用値）と全画面状態を、ルート要素の `data-platform` / `data-fullscreen` へ反映する純関数 `applyPlatform` / `applyFullScreen`（単体テスト可）。`App.tsx` が main / preview（`#preview`）双方のトップなので、ここで一括適用し `onFullScreenChange` を購読する。CSS は `:root[data-platform='darwin']` で macOS 統合タイトルバー（上端バーの `-webkit-app-region: drag`、操作要素の `no-drag`、traffic light 用左余白、全画面時の余白畳み込み）を出し分ける。Windows / Linux は属性が付かず native frame のまま。
- `features/preview/`
...
- ファイル一覧のチェックボックス（行 / 全選択）は `<input type="checkbox">` を `<label class="checkbox-control">` で包み、セル全体（約 34px 幅 × 行高）を hit area にする（行高は据え置き）。セル（`.checkbox-cell`）は click 伝播を止め、行の Open / 通常選択と二重発火しない。input は `appearance: none` で theme token 描画（背景 / 枠 / 角丸 / hover / checked のチェック / `indeterminate` の横棒 / `focus-visible` リング / disabled）し、light/dark・font-size・density に追従。native input semantics（keyboard / aria / `indeterminate`）は維持し、`forced-colors` では native 描画へ戻す。
