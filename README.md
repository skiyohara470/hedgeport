...
- 複数選択と一括操作
- タブによる複数接続の切り替え
- 組み込みテキストエディターと独立ウィンドウのファイルプレビュー
  - ファイルのダブルクリックでプレビューを開く（ディレクトリはペイン内移動）
  - UTF-8、Shift_JIS、EUC-JP
  - 文字コードの自動判定と手動選択
  - プレビューウィンドウ内検索（Ctrl/Cmd+F、前後移動・一致件数・大文字小文字の区別）
  - プレビューは最大 20 MiB、組み込みエディターでの編集は最大 1 MiB
...
- 独立プレビューウィンドウは実ファイル表示とウィンドウ内検索に対応しています。差分（diff）表示は未実装です。
